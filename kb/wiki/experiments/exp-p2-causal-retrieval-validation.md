...
1. Compute pairwise TE/Granger scores on a tractable subset of series (distillation set).
2. Train a directed/asymmetric embedding objective from those soft labels.
...
- Directional retrieval: ranking metrics against held-out TE/Granger labels; A→B vs B→A consistency.
- Geometry health: does training converge to a stable asymmetric space, or collapse to symmetric?
- Downstream: forecast lift over DTW/shape covariates on held-out series.
- Retrieval latency at target scale.
