...
> **Stage: seed.** This wiki is built from the starting sources only (one strategy deck, 5 design/technical notes, p1, ..., p4, px). It is a scaffold for research that will be executed later, not the full scope. See [shared/research-backlog](shared/research-backlog.md) for what is planned, and `CLAUDE.md` → Maturity model for what `seed`/`researched`/`validated` mean.
...
P1, P2, and P3 are an integrated forecasting/scenario stack. P3 is the near-term commercial product, P2 is the research moat around causal covariate retrieval, and P1 is the scalability and quality unlock.
