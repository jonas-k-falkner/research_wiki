...
- **Determinism vs MC-dropout `[resolved 2026-06-30]`.** MC-dropout with a fixed seed is deterministic: same seed → same stochastic passes → same importance estimates. The seeded approach is sufficient for reproducible uncertainty/importance estimation — no stochasticity carve-out needed. `forecast_pipeline` repo context is not required to resolve this.
